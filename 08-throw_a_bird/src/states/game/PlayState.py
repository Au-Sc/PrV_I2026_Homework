...
class PlayState(BaseState):
    def enter(self) -> None:
        ...
        self.bird_centroid = pygame.Vector2()
        ...
        self.world.on_collision_begin(self.bird_collision_check)

    def bird_collision_check(self, body, other) -> None:
    ...
    def update(self, dt: float) -> None:
        ...
        if len(self.bird_list) > 0:
            self.bird_centroid.x = sum(b.body.position.x for b in self.bird_list)/len(self.bird_list)
            self.bird_centroid.y = sum(b.body.position.y for b in self.bird_list)/len(self.bird_list)
        ...
        if self.flinging:
            self.camera_target.update(self.bird_centroid)
            self._update_idle()
        elif self.aiming:
    ...
    def _update_idle(self) -> None:
        if all(
            b.body.velocity.length() < IDLE_LINEAR_SPEED_THRESHOLD
            and abs(b.body.angular_velocity) < IDLE_ANGULAR_SPEED_THRESHOLD
            ...
            if self.idle_frames > IDLE_FRAMES_LIMIT:
                ...
                if len(self.bird_list) > 1:
                    for i in range(1,len(self.bird_list)):
                        self.world.destroy_body(self.bird_list[i].body)
                    del self.bird_list[1:len(self.bird_list)]
                self.idle_frames = 0
                self.bird.reset()
    ...
    def _update_zoom(self, dt: float) -> None:
        ...
        furthest_bird_distance = 0
        for b in self.bird_list:
            d = (self.bird_centroid - b.body.position).length()
            furthest_bird_distance = max(furthest_bird_distance, d)

        #bdr stands for bird distance ratio
        bdr = max(
            1,
            0.1 + furthest_bird_distance * self.camera.zoom / settings.VIRTUAL_WIDTH/2
        )

        target_ratio = max(
            CAMERA_ZOOM_MIN, min(CAMERA_ZOOM_MAX, math.sqrt(bdr * distance / reach))
        )
    ...
    def on_input(self, input_id: str, input_data: InputData) -> None:
        ...
        if input_id == "split" and self.split_available:
            self.split_available = False
            self.bird_split_spawn()
    ...
    def bird_split_spawn(self):
        x = self.bird.body.position.x
        y = self.bird.body.position.y
        m = self.bird.mass

        top_bird = Bird(self.world, x, y - self.bird.radius * 2.1)
        v = self.bird.body.velocity.rotate(-10)
        top_bird.body.apply_impulse(v.x * m, v.y * m)

        bottom_bird = Bird(self.world, x, y + self.bird.radius * 2.1)
        v = self.bird.body.velocity.rotate(10)
        bottom_bird.body.apply_impulse(v.x * m, v.y * m)

        self.bird_list.append(top_bird)
        self.bird_list.append(bottom_bird)
