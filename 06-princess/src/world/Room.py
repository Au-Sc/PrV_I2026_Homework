...
from src.BossFight import BossFight
...
class Room:
    def __init__(
        ...
        self.can_generate_chest = params.get("can_generate_chest", False)
        self.single_door_render: Optional(str) = params.get("enter_from", None)
        
        self.boss_fight: Optional[BossFight] = None

        if not params.get("boss_room", False):
            self._generate_entities()
            self._generate_objects()
        else:
            self.boss_fight = BossFight(self)
    ...
    def update(self, dt: float) -> None:
        ...
        for projectile in list(self.projectiles):
            ...
            for entity in self.entities:
                ...
                if (
                    not entity.dead 
                    and not entity.invulnerable
                    and not entity.inmune
    ...
    def take_adjacent_object(self, player: TypeVar("Player")) -> None:
        """
        ...
        for obj in self.objects:
            ...
            if adjacent:
                if obj.type == "chest":
                    player.bow = Bow(player.state_machine.current.dungeon)
                else:
    ...
    def _generate_objects(self) -> None:
        ...
        if self.can_generate_chest: #and random.randint(1,7) == 1:
            x = random.randint(3, self.width - 2)
            y = random.randint(3, self.height - 2)
            self.objects.append(
                GameObject(GAME_OBJECT_DEFS["chest"], x * 16, y * 16)
            )
            self.can_generate_chest = False
    ...
    def render(
        ...
        for doorway in self.doorways:
            if (
                not self.single_door_render 
                or doorway.direction == self.single_door_render
            ):
        ...
        if self.boss_fight:
            self.boss_fight.render(surface, camera_offset_x, camera_offset_y)
