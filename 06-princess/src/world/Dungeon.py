...
class Dungeon:
    def __init__(
        ...
        self.current_room = Room(
                ...
                self.on_game_over,
                {
                    "can_generate_chest":self.chest_available
                }
            )
    ...
    def begin_shifting(self, shift_x: float, shift_y: float) -> None:
        ...
        """
        self.shifting = True
        self.chest_available = self.current_room.can_generate_chest
        ...
        if self.player.bow: #and random.randint(1,7) == 1: #make the boss room
            self.next_room = Room(
                self.player,
                self.on_game_over,
                {
    ...
    def _finish_shifting_and_place_player(self) -> None:
        ...
        if self.current_room.boss_fight:
            self.current_room.boss_fight.room_ready()
